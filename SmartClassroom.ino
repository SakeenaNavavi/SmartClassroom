...
#define relayLock 12  // New relay pin for the lock
...
void setup() {
  ...
  pinMode(relayLock, OUTPUT);  // Initialize lock relay pin

  digitalWrite(relayLock, LOW); // Lock is initially locked
...
void loop() {
  if (!authorized) {
    ...
    if (getFingerPrint() != -1) {
      ...
      digitalWrite(relayLock, HIGH); // Unlock the lock
      delay(ACCESS_DELAY);
      digitalWrite(relayLock, LOW); // Lock the lock again
      lcd.clear();
      lcd.setCursor(4, 0);
      lcd.print("COUNTER");
      lcd.setCursor(0, 1);
